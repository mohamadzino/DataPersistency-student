...
select medewerkers.mnr, cursussen.code
from medewerkers
...
group by medewerkers.mnr, cursussen.code
having count(*) > 1;
...
select max(gbdatum) - min(gbdatum) as verschil, avg(extract(year from age(gbdatum))) as gemiddeld
from medewerkers;
